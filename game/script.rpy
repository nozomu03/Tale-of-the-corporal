...
label start:
   ...
   # show para_text("aaaaaa")
    ...
    $SoundPlayer("teleclick.ogg", 1.0)
    adjutant "오늘은 되게 칼같이 하네. {w}평소에는 시간 안 지키고 아무때나 걸었잖아."
    ...
    "지난 경험이. {w}살아왔던 시간이. {w}내 발목을 붙잡아 끌며 전진하는 것을 방해했다. {w}과거 겪었던 일과 닮은, 혹은 동일한 시간이 내게 찾아올 때마다. {w}나는 움츠러들 수 밖에 없었고 그에 따른 정당한 비판을 받아냈다."
    hide main_atten
    $SoundPlayer("walk_slow.ogg", 2.0)
...
#   $SoundPlayer("teleclick.ogg", 1.0) 
   # "."
    ...
    extend "\n그로 인한 어떠한 불이익도 받지 않았으나 그 사실은 지금까지도 잔류하여 시시때때로 이 마음을 혼탁케 해 왔다."
    ...
    main "저도 그래버릴까요... {w}연속 근무 때문에 피곤한데..."
    wol "평소엔 항상 일찍 일어났죠?"
    main "예."
